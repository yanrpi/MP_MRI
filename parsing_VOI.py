...
class ParseVOI(object):
    ...
    def list_of_dicts_all_images(self,filepaths):
        '''generates a list of dicts for all patients in a root directory
    ...
    def list_of_dicts(self,filepath):
        '''generates a list of dicts, one for each .voi file in the directory
    ...
    def BBox_from_position(self,path):
        '''
        take locations obtained 
        :return: dict {slice:(filename,xmin,ymin,xmax,ymax)}
        '''
        ...
        for slice in dict.keys():
            values=dict[slice]
            category=values[0] # category is the name of the file
    ...
    def get_ROI_slice_loc(self,path=None):
        '''
        performs simple string matching using '# slice number' to find the line location of start and end of each slice.
        :return: dict of {slice number:(filename, start_line_number, end_line_number)}
